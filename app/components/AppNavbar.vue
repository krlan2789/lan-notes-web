...
<template>
	<div class="fixed w-full top-0 z-100 bg-surface-100">
		<Menubar
			:model="items"
			class="w-full max-w-7xl mx-auto rounded-none! bg-surface-100"
			:pt="{
				itemContent: {
					class: [
						'cursor-pointer px-3 py-2 font-medium text-surface-700 transition duration-200 hover:text-primary-800',
					],
				},
				item: {
					class: ['py-0 my-0'],
		...
		>
			<template #start>
				<NuxtLink to="/" class="block py-2 text-2xl sm:text-3xl font-semibold text-primary-500">
					<p class="flex flex-row items-center w-auto h-full gap-2 align-middle font-orbitron" translate="no">
						<span class="size-7">
							<svg class="size-full" viewBox="0 0 400.01 400.009" fill="none" xmlns="http://www.w3.org/2000/svg">
								...
								<g id="Logo LAN">
									<g id="Logo">
										...
										<path
											...
											fill="currentColor"
											transform="translate(81.967 167.213)"
										/>
									</g>
								</g>
							</svg>
				...
				<Button @click="eventBus.$emit(DialogSearchEventName.OnShow)" variant="outlined" class="px-2 py-1.5 w-auto">
					<i class="pi pi-search text-xs"></i>
					Find
					<Chip icon="pi pi-microsoft" class="py-[0.5px] px-1.5 gap-px bg-primary-200">
						<template #default>
			...
			<template #item="{ item, props, hasSubmenu }">
				<NuxtLink :to="item.route" rel="noopener noreferrer">
