...
<template>
	<Panel
		...
		:pt="{
			header: {
				class: 'bg-surface-0 p-0 mx-0 mb-6 border-none rounded-none shadow-none',
			},
			root: {
				class: 'bg-surface-0 p-0 m-0 border-none rounded-none shadow-none',
			},
		}"
	>
		<template #header>
		...
		<div class="flex flex-wrap gap-2 justify-center bg-surface-0 w-full text-center">
			<span v-for="(tag, index) of availableTags" :key="tag">
				...
				<label
					:for="'tag-' + tag + '-' + index"
					class="flex items-center justify-center text-xs py-[3px] px-2 font-medium select-none cursor-pointer rounded-full border-[1.5px] border-surface-700 text-surface-700 transition-colors duration-200 ease-in-out peer-checked:bg-surface-700 peer-checked:text-surface-100"
				>
					#{{ tag }}
				</label>
			</span>
